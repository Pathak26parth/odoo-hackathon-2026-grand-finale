      ...
      <div className="space-y-3 pt-1">
        {data.length === 0 ? (
          <div className="py-8 text-center text-xs text-slate-400">
            No department expenditures recorded for selected filter.
          </div>
        ) : (
          data.map((item, idx) => {
            const widthPct = maxAmount > 0 ? Math.round((item.amount / maxAmount) * 100) : 0;
            return (
              <div key={idx} className="space-y-1">
                ...
                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${item.amount > 0 ? Math.max(widthPct, 2) : 0}%` }}
                  />
                </div>
              </div>
            );
